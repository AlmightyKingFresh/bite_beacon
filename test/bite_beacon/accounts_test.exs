defmodule BiteBeacon.AccountsTest do
  ...
  import BiteBeacon.AccountsFixtures
  alias BiteBeacon.Accounts.{Vendor, VendorToken}

  describe "get_vendor_by_email/1" do
    test "does not return the vendor if the email does not exist" do
      refute Accounts.get_vendor_by_email("unknown@example.com")
    end

    test "returns the vendor if the email exists" do
      %{id: id} = vendor = vendor_fixture()
      assert %Vendor{id: ^id} = Accounts.get_vendor_by_email(vendor.email)
    end
  end

  describe "get_vendor_by_email_and_password/2" do
    test "does not return the vendor if the email does not exist" do
      refute Accounts.get_vendor_by_email_and_password("unknown@example.com", "hello world!")
    end

    test "does not return the vendor if the password is not valid" do
      vendor = vendor_fixture()
      refute Accounts.get_vendor_by_email_and_password(vendor.email, "invalid")
    end
    ...
    test "returns the vendor if the email and password are valid" do
      ...
      assert %Vendor{id: ^id} =
               Accounts.get_vendor_by_email_and_password(vendor.email, valid_vendor_password())
    end
  end
  ...
  describe "get_vendor!/1" do
    test "raises if id is invalid" do
      assert_raise Ecto.NoResultsError, fn ->
        Accounts.get_vendor!(-1)
      end
    end

    test "returns the vendor with the given id" do
      %{id: id} = vendor = vendor_fixture()
      assert %Vendor{id: ^id} = Accounts.get_vendor!(vendor.id)
    end
  end

  describe "register_vendor/1" do
    test "requires email and password to be set" do
      {:error, changeset} = Accounts.register_vendor(%{})
    ...
    test "validates email and password when given" do
      {:error, changeset} = Accounts.register_vendor(%{email: "not valid", password: "not valid"})
    ...
    test "validates maximum values for email and password for security" do
      too_long = String.duplicate("db", 100)
      {:error, changeset} = Accounts.register_vendor(%{email: too_long, password: too_long})
      assert "should be at most 160 character(s)" in errors_on(changeset).email
      assert "should be at most 72 character(s)" in errors_on(changeset).password
    end
    ...
    test "validates email uniqueness" do
      ...
      {:error, changeset} = Accounts.register_vendor(%{email: email})
      assert "has already been taken" in errors_on(changeset).email

      # Now try with the upper cased email too, to check that email case is ignored.
      {:error, changeset} = Accounts.register_vendor(%{email: String.upcase(email)})
      assert "has already been taken" in errors_on(changeset).email
    end

    test "registers vendors with a hashed password" do
      email = unique_vendor_email()
      {:ok, vendor} = Accounts.register_vendor(valid_vendor_attributes(email: email))
  ...
  describe "change_vendor_registration/2" do
    test "returns a changeset" do
      assert %Ecto.Changeset{} = changeset = Accounts.change_vendor_registration(%Vendor{})
      assert changeset.required == [:password, :email]
    end
    ...
    test "allows fields to be set" do
      ...
      changeset =
        Accounts.change_vendor_registration(
          %Vendor{},
  ...
  describe "change_vendor_email/2" do
    test "returns a vendor changeset" do
      assert %Ecto.Changeset{} = changeset = Accounts.change_vendor_email(%Vendor{})
      assert changeset.required == [:email]
    end
  end
  ...
  describe "apply_vendor_email/3" do
    ...
    test "requires email to change", %{vendor: vendor} do
      {:error, changeset} = Accounts.apply_vendor_email(vendor, valid_vendor_password(), %{})
      assert %{email: ["did not change"]} = errors_on(changeset)
    end

    test "validates email", %{vendor: vendor} do
      {:error, changeset} =
        Accounts.apply_vendor_email(vendor, valid_vendor_password(), %{email: "not valid"})
    ...
    test "validates maximum value for email for security", %{vendor: vendor} do
      ...
      {:error, changeset} =
        Accounts.apply_vendor_email(vendor, valid_vendor_password(), %{email: too_long})
    ...
    test "validates email uniqueness", %{vendor: vendor} do
      ...
      {:error, changeset} = Accounts.apply_vendor_email(vendor, password, %{email: email})
    ...
    test "validates current password", %{vendor: vendor} do
      {:error, changeset} =
        Accounts.apply_vendor_email(vendor, "invalid", %{email: unique_vendor_email()})
    ...
    test "applies the email without persisting it", %{vendor: vendor} do
      ...
      {:ok, vendor} =
        Accounts.apply_vendor_email(vendor, valid_vendor_password(), %{email: email})

      assert vendor.email == email
      assert Accounts.get_vendor!(vendor.id).email != email
    end
  end
    ...
    test "sends token through notification", %{vendor: vendor} do
      token =
        extract_vendor_token(fn url ->
          Accounts.deliver_vendor_update_email_instructions(vendor, "current@example.com", url)
        end)
  ...
  describe "update_vendor_email/2" do
    ...
    test "updates the email with a valid token", %{vendor: vendor, token: token, email: email} do
      assert Accounts.update_vendor_email(vendor, token) == :ok
    ...
    test "does not update email with invalid token", %{vendor: vendor} do
      assert Accounts.update_vendor_email(vendor, "oops") == :error
    ...
    test "does not update email if vendor email changed", %{vendor: vendor, token: token} do
      assert Accounts.update_vendor_email(%{vendor | email: "current@example.com"}, token) ==
               :error
    ...
    test "does not update email if token expired", %{vendor: vendor, token: token} do
      ...
      assert Accounts.update_vendor_email(vendor, token) == :error
      assert Repo.get!(Vendor, vendor.id).email == vendor.email
      assert Repo.get_by(VendorToken, vendor_id: vendor.id)
    end
  end
  ...
  describe "change_vendor_password/2" do
    test "returns a vendor changeset" do
      assert %Ecto.Changeset{} = changeset = Accounts.change_vendor_password(%Vendor{})
      assert changeset.required == [:password]
    end
  ...
  describe "update_vendor_password/3" do
    ...
    test "validates password", %{vendor: vendor} do
      {:error, changeset} =
        Accounts.update_vendor_password(vendor, valid_vendor_password(), %{
          password: "not valid",
          password_confirmation: "another"
        })
    ...
    test "validates maximum values for password for security", %{vendor: vendor} do
      ...
      {:error, changeset} =
        Accounts.update_vendor_password(vendor, valid_vendor_password(), %{password: too_long})
    ...
    test "validates current password", %{vendor: vendor} do
      {:error, changeset} =
        Accounts.update_vendor_password(vendor, "invalid", %{password: valid_vendor_password()})
  ...
  describe "get_vendor_by_session_token/1" do
    ...
    test "returns vendor by token", %{vendor: vendor, token: token} do
      assert session_vendor = Accounts.get_vendor_by_session_token(token)
      assert session_vendor.id == vendor.id
    end

    test "does not return vendor for invalid token" do
      refute Accounts.get_vendor_by_session_token("oops")
    end

    test "does not return vendor for expired token", %{token: token} do
      {1, nil} = Repo.update_all(VendorToken, set: [inserted_at: ~N[2020-01-01 00:00:00]])
      refute Accounts.get_vendor_by_session_token(token)
    end
  end
  ...
  describe "delete_vendor_session_token/1" do
    test "deletes the token" do
      vendor = vendor_fixture()
      token = Accounts.generate_vendor_session_token(vendor)
      assert Accounts.delete_vendor_session_token(token) == :ok
      refute Accounts.get_vendor_by_session_token(token)
    end
  end
  ...
  describe "confirm_vendor/1" do
    ...
    test "confirms the email with a valid token", %{vendor: vendor, token: token} do
      assert {:ok, confirmed_vendor} = Accounts.confirm_vendor(token)
    ...
    test "does not confirm with invalid token", %{vendor: vendor} do
      assert Accounts.confirm_vendor("oops") == :error
    ...
    test "does not confirm email if token expired", %{vendor: vendor, token: token} do
      {1, nil} = Repo.update_all(VendorToken, set: [inserted_at: ~N[2020-01-01 00:00:00]])
      assert Accounts.confirm_vendor(token) == :error
      refute Repo.get!(Vendor, vendor.id).confirmed_at
      assert Repo.get_by(VendorToken, vendor_id: vendor.id)
    end
  end
  ...
  describe "reset_vendor_password/2" do
    ...
    test "validates password", %{vendor: vendor} do
      {:error, changeset} =
        Accounts.reset_vendor_password(vendor, %{
          password: "not valid",
          password_confirmation: "another"
        })
    ...
    test "validates maximum values for password for security", %{vendor: vendor} do
      too_long = String.duplicate("db", 100)
      {:error, changeset} = Accounts.reset_vendor_password(vendor, %{password: too_long})
      assert "should be at most 72 character(s)" in errors_on(changeset).password
    end

    test "updates the password", %{vendor: vendor} do
      {:ok, updated_vendor} =
        Accounts.reset_vendor_password(vendor, %{password: "new valid password"})
    ...
    test "deletes all tokens for the given vendor", %{vendor: vendor} do
      _ = Accounts.generate_vendor_session_token(vendor)
      {:ok, _} = Accounts.reset_vendor_password(vendor, %{password: "new valid password"})
      refute Repo.get_by(VendorToken, vendor_id: vendor.id)
    end
  end
